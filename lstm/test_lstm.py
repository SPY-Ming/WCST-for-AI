...
class Agent_wsct(object):
    def __init__(self):
        ...
        with self.graph_lstm.as_default():
            saver2 = tf.train.import_meta_graph("./model/lstm_6dim_v2.meta")
            saver2.restore(self.sess_lstm, "./model/lstm_6dim_v2")
...
#         print(t)
    ...
    for x,y in dataset[-5:]:
        ...
        res = a.sess_lstm.run(tf.argmax(a.lstm_pred,1),feed_dict={a.lstm_x:[x]})
        
        print("{} \t {} \t {}".format(x,res,np.argmax(y, 0)))
#         print(a.sess_lstm.run(tf.argmax([0,0,1],0)),np.argmax([0,0,1],0))
